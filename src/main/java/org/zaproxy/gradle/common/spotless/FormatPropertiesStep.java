...
public final class FormatPropertiesStep {
    ...
    static String format(String rawUnix) throws Exception {
        ...
        Map<InvalidPropertyReason, List<String>> invalidProperties = new HashMap<>();
        properties
                .getKeys()
                .forEachRemaining(
                        key -> {
                            if ("".equals(key)) {
                                invalidProperties.putIfAbsent(
                                        LEADING_SPACE_IN_NAME, new ArrayList<>());
                                invalidProperties
                                        .get(LEADING_SPACE_IN_NAME)
                                        .add(properties.getString(key).split("=", 2)[0].trim());
                            } else if (!properties.getLayout().isSingleLine(key)) {
                                invalidProperties.putIfAbsent(DUPLICATE, new ArrayList<>());
                                invalidProperties.get(DUPLICATE).add(key);
                            }
                        });
